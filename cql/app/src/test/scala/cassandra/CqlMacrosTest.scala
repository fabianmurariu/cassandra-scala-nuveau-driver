...
//    }
    ...
    "Matcher macros" should {
      "produce matchers " in {
        Cql2[Person](p => p.name == "ikea" && p.age < 18) === And(Eq("name", "ikea"), Lt("age", 18))
//        Cql2[Person](p => p.name == "ikea" && p.age < 18 && p.age >= 5) === And(Eq("name", "ikea"), And(Lt("age", 18), GtEq("age", 5)))
        Cql2[Person](_.name == "ikea") === Eq("name", "ikea")
        Cql2[Person](_.age > 24) === Gt("age", 24)
        Cql2[Person](_.age < 2) === Lt("age", 2)
        Cql2[Person](_.age <= 4) === LtEq("age", 4)
        Cql2[Person](_.age >= 25) === GtEq("age", 25)
      }
    }
//  }
